...
def _render_one_by_one(questions: list, interest: str, grade: str, lt: str):
    ...
    if idx < total:
        ...
        st.markdown(
            ...
            f'<div style="background:#3b82f6;width:{int(idx/total*100)}%;height:100%;border-radius:999px;"></div>'
            f'</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            f'<div style="font-size:12px;color:#6b7280;margin-bottom:8px;">질문 {idx+1} / {total}</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            ...
            f'<div style="font-size:13px;font-weight:700;color:#3b82f6;margin-bottom:6px;">Q{idx+1}</div>'
...
def _render_refined(refined: dict):
    ...
    if keywords:
        tags = "".join(
            f'<span style="background:#dbeafe;color:#1d4ed8;border-radius:20px;padding:3px 10px;'
            f'font-size:12px;font-weight:600;margin:2px;">{kw}</span>'
            for kw in keywords
        )
